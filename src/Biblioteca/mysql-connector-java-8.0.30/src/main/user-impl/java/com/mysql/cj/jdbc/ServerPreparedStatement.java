...
public class ServerPreparedStatement extends ClientPreparedStatement {
    ...
    protected long[] executeBatchSerially(int batchTimeout) throws SQLException {
        synchronized (checkClosed().getConnectionMutex()) {
            ...
            BindValue[] oldBindValues = ((PreparedQuery) this.query).getQueryBindings().getBindValues();
            ...
            try {
                                    ...
                                    if (previousBindValuesForBatch != null) {
                                        for (int j = 0; j < parameterBindings.length; j++) {
                                            if (parameterBindings[j].getMysqlType() != previousBindValuesForBatch[j].getMysqlType()) {
                                                ((PreparedQuery) this.query).getQueryBindings().getSendTypesToServer().set(true);
                                                break;
                                            }
                                        }
                                    }
                ...
                ((PreparedQuery) this.query).getQueryBindings().setBindValues(oldBindValues);
                ((PreparedQuery) this.query).getQueryBindings().getSendTypesToServer().set(true);
    ...
    protected BindValue getBinding(int parameterIndex, boolean forLongData) throws SQLException {
        synchronized (checkClosed().getConnectionMutex()) {
            ...
            return ((PreparedQuery) this.query).getQueryBindings().getBinding(i, forLongData);
        }
    }
